	...
	for (int j = 0; j < borderPoints.size(); j++) {
		...
		if (points[i].IsNeigh(borderPoints[j])) {
			...
			for (int k = 0; k < t.size(); k++) {
				double A1 = 0, A2 = 0, B1 = 0, B2 = 0;		//differ
				t[k]->GetDiff(points[i], A1, B1);
				t[k]->GetDiff(borderPoints[j], A2, B2);
...
void Model::StartSolving() {
	...
	//update points in triangles
	for (int i = 0; i < points.size(); i++) {
		for (int j = 0; j < triangles.size(); j++) {
			if (triangles[j].GetP1() == points[i])
				triangles[j].GetpP1()->SetP(points[i].P());
			else if (triangles[j].GetP2() == points[i])
				triangles[j].GetpP2()->SetP(points[i].P());
			else if (triangles[j].GetP3() == points[i])
				triangles[j].GetpP3()->SetP(points[i].P());
		}
	}
